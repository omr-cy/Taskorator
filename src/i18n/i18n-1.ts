...
const translationsZH: Record<TranslationKey, string> = {
    ...
    'settings.template.tags.desc': '支持：#every-day (每天), #every(mon, sun) (特定周几), #every(jan(1,15)) (特定月及日期), #every workday (工作日)',
    'settings.template.vars.desc': '{{date}} - 日期, {{dateWithIcon}} - 带图标日期, {{weekday}} - 星期, {{yearProgress}} - 年进度, {{monthProgress}} - 月进度, {{time}} - 时间',
...
const translationsEN: Record<TranslationKey, string> = {
    ...
    'settings.template.tags.desc': 'Support adding: #every-day (Every day), #every(mon, sun) (Specific days), #every(jan(1,15)) (Month with specific days), #every workday (Workdays)',
    'settings.template.vars.desc': '{{date}} - Date, {{dateWithIcon}} - Date with icon, {{weekday}} - Weekday, {{yearProgress}} - Year progress, {{monthProgress}} - Month progress, {{time}} - Current time',
...
const translationsAR: Record<TranslationKey, string> = {
    ...
    'settings.template.tags.desc': 'دعم الإضافة: #every-day (يومياً), #every(mon, sun) (أيام محددة), #every(jan(1,15)) (أيام محددة في الشهر), #every workday (أيام العمل)',
    'settings.template.vars.desc': '{{date}} - التاريخ, {{dateWithIcon}} - التاريخ مع أيقونة, {{weekday}} - اليوم, {{yearProgress}} - تقدم السنة, {{monthProgress}} - تقدم الشهر, {{time}} - الوقت الحالي',
